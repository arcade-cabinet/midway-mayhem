/**
 * ErrorModal.browser.test.tsx
 *
 * Real-browser tests for <ErrorModal>:
 *   - visibility / data-error-count
 *   - context + message rendering on reportError
 *   - stack trace element in DOM
 *   - dismiss button removes modal
 *   - multi-error count in subtitle
 *   - Escape key dismisses (real browser event loop)
 *   - role="alertdialog" and focus management
 */
...
describe('<ErrorModal />', () => {
  ...
  it('has role="alertdialog" and aria-modal when visible', async () => {
    await act(async () => {
  ...
  it('has aria-label="Mayhem Halted"', async () => {
    await act(async () => {
      reportError(new Error('Test failure'), 'test');
    });
    render(<ErrorModal />);
  ...
  it('has aria-describedby pointing at the message element', async () => {
    await act(async () => {
