...
export function seedContent(world: World, seed: number, opts: Options = {}): void {
	...
	for (let i = 0; i < obstacleCount; i++) {
		...
		// Critter-specific: pick a random animal kind.
		const critterKind =
			kind === 'critter' ? (CRITTER_KINDS[rng.int(0, CRITTER_KINDS.length)] ?? 'cow') : ('' as const);
		// Hammer-specific: randomize swing phase so hammers animate out-of-sync.
		const swingPhase = kind === 'hammer' ? rng.next() * Math.PI * 2 : 0;
		world.spawn(
			Obstacle({ kind, distance, lateral, consumed: false, critterKind, swingPhase, fleeStartedAt: 0, fleeDir: 0 }),
		);
	}
