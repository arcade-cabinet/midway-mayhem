...
export function useShake() {
  ...
  return {
    ...
    sample(t: number, dt: number) {
      ...
      const y =
        Math.sin(t * c.bob.freq) * c.bob.amp +
        Math.sin(t * c.speed.freq * 6.28) * c.speed.amp * 0.3 +
        (Math.random() - 0.5) * c.crash.amp;
      const x = Math.sin(t * 1.7) * c.bob.amp * 0.4 + (Math.random() - 0.5) * c.crash.amp * 0.5;
      return { x, y };
    },
  };
}
