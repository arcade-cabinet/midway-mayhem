/**
 * Post-processing stack — runs on every device. No perf-tier branches.
 * If this tanks FPS, we fix perf; we don't hide the problem.
 *
 * Effects (in order):
 *   - ACES Filmic tone mapping (from reference)
 *   - Zone-crossfading hue/saturation + brightness/contrast (from reference)
 *   - Bloom (v2 baseline, tuned)
 *   - Film grain noise (from reference)
 *   - Vignette (v2 baseline)
 *
 * Merge: v2 had Bloom + Vignette only. Reference added ToneMapping,
 * HueSaturation, BrightnessContrast, Noise, and per-zone color grading.
 * SpeedFX (chromatic aberration + reactive vignette) remains disabled —
 * JSON.stringify on Vignette/ChromaticAberration args crashes with a
 * circular-reference on this Three/R3F version combo. The SpeedFX component
 * exists in src/render/cockpit/SpeedFX.tsx for future re-enabling once
 * the postprocessing version is updated.
 */
...
export function PostFX() {
  ...
  return (
    <EffectComposer multisampling={0}>
      ...
      {/* SpeedFX disabled — see module comment for why */}
      {/* <SpeedFX /> */}
    </EffectComposer>
  );
}
