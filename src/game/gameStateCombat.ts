...
export function applyCrashAction(heavy: boolean, set: SetFn, get: GetFn): void {
  const s = get();
  if (s.permadeath) {
    set({ sanity: 0, crashes: s.crashes + 1, speedMps: s.speedMps * 0.55, gameOver: true, running: false });
    hapticsBus.fire('game-over');
    return;
  }
  const sanity = Math.max(0, s.sanity - (heavy ? 25 : 10));
  set({ sanity, crashes: s.crashes + 1, speedMps: s.speedMps * 0.55, gameOver: sanity <= 0, running: sanity > 0 });
...
export function applyPickupAction(kind: 'ticket' | 'boost' | 'mega', set: SetFn, get: GetFn): void {
  ...
  if (kind === 'ticket') {
    set({ crowdReaction: s.crowdReaction + Math.round(50 * cleanBonus), ticketsThisRun: s.ticketsThisRun + 1 });
    ...
    set({ megaBoostUntil: now + 3500, crowdReaction: s.crowdReaction + Math.round(200 * cleanBonus) });
