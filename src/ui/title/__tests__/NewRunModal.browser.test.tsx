/**
 * NewRunModal browser component tests.
 * Ported from reference/src/hud/__tests__/ui/NewRunModal.test.tsx
 */
...
describe('NewRunModal', () => {
  it('renders seed input, shuffle, difficulty grid, and play', () => {
    ...
    expect(screen.getByTestId('seed-phrase-input')).toBeTruthy();
    expect(screen.getByTestId('seed-phrase-shuffle')).toBeTruthy();
    expect(screen.getByTestId('difficulty-grid')).toBeTruthy();
    expect(screen.getByTestId('new-run-play')).toBeTruthy();
  });

  it('renders all 6 difficulty tiles', () => {
    render(<NewRunModal onPlay={vi.fn()} onClose={vi.fn()} />);
    expect(screen.getByTestId('difficulty-tile-silly')).toBeTruthy();
    expect(screen.getByTestId('difficulty-tile-kazoo')).toBeTruthy();
    expect(screen.getByTestId('difficulty-tile-plenty')).toBeTruthy();
    expect(screen.getByTestId('difficulty-tile-ultra-honk')).toBeTruthy();
    expect(screen.getByTestId('difficulty-tile-nightmare')).toBeTruthy();
    expect(screen.getByTestId('difficulty-tile-ultra-nightmare')).toBeTruthy();
  });
